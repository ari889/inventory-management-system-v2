    ...
    <div class="card text-white bg-primary">
        ...
        <div class="card-body">
            <h4 class="text-center text-white">
                {{ number_format($sale[0]->grand_total - $sale[0]->tax - ($purchase[0]->grand_total - $purchase[0]->tax), 2, '.', ',') }}
            </h4>
            <p class="text-center"> (Sale {{ number_format($sale[0]->grand_total, 2, '.', ',') }} - Tax
                {{ number_format($sale[0]->tax, 2, '.', ',') }})</p>
            <p class="text-center"> (Purchase {{ number_format($purchase[0]->grand_total, 2, '.', ',') }} - Tax
                {{ number_format($purchase[0]->tax, 2, '.', ',') }})</p>
        </div>
    </div>
</div>
    ...
    @foreach ($warehouse_name as $key => $name)
        <div class="col-xl-4 col-sm-6">
            <div class="card text-white bg-primary">
                ...
                <div class="card-body">
                    <h4 class="text-center text-white">
                        {{ number_format($warehouse_sale[$key][0]->grand_total - $warehouse_purchase[$key][0]->grand_total, 2, '.', ',') }}
                    </h4>
                    <p class="text-center"> (Sale
                        {{ number_format($warehouse_sale[$key][0]->grand_total, 2, '.', ',') }}
                        - Purchase {{ number_format($warehouse_purchase[$key][0]->grand_total, 2, '.', ',') }})</p>

                    <hr style="border-color: white;">

                    <h4 class="text-center text-white">
                        {{ number_format($warehouse_sale[$key][0]->grand_total - $warehouse_sale[$key][0]->tax - ($warehouse_purchase[$key][0]->grand_total - $warehouse_purchase[$key][0]->tax), 2, '.', ',') }}
                    </h4>
                    <p class="text-center"> (Net Sale
                        {{ number_format($warehouse_sale[$key][0]->grand_total - $warehouse_sale[$key][0]->tax, 2, '.', ',') }}
                        - Net Purchase
                        {{ number_format($warehouse_purchase[$key][0]->grand_total - $warehouse_purchase[$key][0]->tax, 2, '.', ',') }})
                    </p>
