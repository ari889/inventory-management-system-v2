                    ...
                    <div class="row">
                            ...
                            <table class="table table-bordered" id="product-list">
                                    ...
                                    @if (!$purchase->purchase_products->isEmpty())
                                        @foreach ($purchase->purchase_products as $key => $purchase_product)
                                            <tr>
                                                @php
                                                    $tax = DB::table('taxes')
                                                        ->where('rate', $purchase_product->pivot->tax_rate)
                                                        ->first();
                                                    $units = DB::table('units')
                                                        ->where('base_unit', $purchase_product->unit_id)
                                                ...
                                                <td><input type="text" class="form-control qty text-center"
                                                        ...
                                                        value="{{ $purchase_product->pivot->qty }}"></td>
                                                <td class="received-product-qty d-none"><input type="text"
                                                        class="form-control received text-center"
                                                        name="products[{{ $key + 1 }}][received]"
                                                        value="{{ $purchase_product->pivot->received }}"></td>

                                                <td class="net_unit_cost text-right">
                                                    {{ number_format((float) $purchase_product->pivot->net_unit_cost, 2, '.', '') }}
                                                </td>
                                                <td class="discount text-right">
                                                    {{ number_format((float) $purchase_product->pivot->discount, 2, '.', '') }}
                                                </td>
                                                <td class="tax text-right">
                                                    {{ number_format((float) $purchase_product->pivot->tax, 2, '.', '') }}
                                                </td>
                                                <td class="sub-total text-right">
                                                    {{ number_format((float) $purchase_product->pivot->total, 2, '.', '') }}
                                                </td>
                                                ...
                                                <input type="hidden" class="net_unit_cost"
                                                    name="products[{{ $key + 1 }}][net_unit_cost]"
                                                    value="{{ $purchase_product->pivot->net_unit_cost }}">
                                                <input type="hidden" class="discount-value"
                                                    name="products[{{ $key + 1 }}][discount]"
                                                    value="{{ $purchase_product->pivot->discount }}">
                                                <input type="hidden" class="tax-rate"
                                                    name="products[{{ $key + 1 }}][tax_rate]"
                                                    value="{{ $purchase_product->pivot->tax_rate }}">
                                                @if ($tax)
                                ...
                                <tfoot class="bg-primary">
                                    ...
                                    <th id="total-discount" class="text-right">
                                        {{ number_format($purchase->total_discount, 2, '.', ',') }}</th>
                                    <th id="total-tax" class="text-right">
                                        {{ number_format($purchase->total_tax, 2, '.', ',') }}</th>
                                    <th id="total" class="text-right">
                                        {{ number_format($purchase->total_cost, 2, '.', ',') }}</th>
                                    <th></th>
                                </tfoot>
                            </table>
                        </div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" class="form-control" name="order_discount" id="order_discount"
                                placeholder="Order discount" value="{{ $purchase->order_discount }}">
                        </div>
                        <div class="col-md-4 mb-3">
                            <label for="shipping_cost" class="form-label">Shipping Cost</label>
                            <input type="text" class="form-control" name="shipping_cost" id="shipping_cost"
                                placeholder="Shipping cost" value="{{ $purchase->shipping_cost }}">
                        </div>
                        <div class="col-md-12 mb-3">
                            <label for="note" class="form-label">Note</label>
                            <textarea rows="10" cols="10" class="form-control" name="note" id="note"
                                placeholder="Enter order note">{{ $purchase->note }}</textarea>
                        </div>
                        <div class="col-md-12">
                            <table class="table table-bordered">
                                <thead class="bg-primary text-white">
                                    ...
                                    <td><strong>Order Tax</strong><span class="float-end"
                                            id="order_total_tax">0.00</span>
                                    </td>
                                    ...
                                    <td><strong>Grand Total</strong><span class="float-end"
                        ...
                        <div class="col-md-12 text-center">
                            ...
                            <button type="button" class="btn btn-primary btn-sm me-2" id="save-btn" data-bs-toggle="tooltip"
                                data-bs-placement="top" title="Save Data">Save</button>
                        </div>
                    </div>
                </form>
            </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <x-form.textbox labelName="Unit Discount" name="edit_discount" required="required"
                        col="col-md-12 mb-3" />
                    <x-form.textbox labelName="Unit Cost" name="edit_unit_cost" required="required" col="col-md-12 mb-3" />
                    @php
                        $tax_name_all[] = 'No Tax';
                        $tax_rate_all[] = 0;
                        foreach ($taxes as $tax) {
        ...
        $(document).ready(function() {
            ...
            $('#product_code_name').autocomplete({
                source: function(request, response) {
                    $.ajax({
                        url: '{{ route('product.autocomplete.search') }}',
                        type: "POST",
                        dataType: "JSON",
                        data: {
                            _token: _token,
            ...
            for (rowindex = 0; rowindex <= rownumber; rowindex++) {
                product_cost.push(parseFloat($('#product-list tbody tr:nth-child(' + (rowindex + 1) + ')').find(
                    '.product-cost').val()));
                ...
                tax_method.push($('#product-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('.tax-method')
                    .val());
                temp_unit_name = $('#product-list tbody tr:nth-child(' + (rowindex + 1) + ')').find(
                    '.purchase-unit').val().split(',');
                unit_name.push($('#product-list tbody tr:nth-child(' + (rowindex + 1) + ')').find('.purchase-unit')
                    .val());
