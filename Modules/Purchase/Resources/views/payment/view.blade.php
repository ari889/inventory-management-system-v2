        ...
        <tr>
            ...
            <td class="text-center">
                <button type="button" class="btn btn-primary btn-sm mr-3 edit-payment" data-id="{{ $payment->id }}"
                    data-purchaseid="{{ $payment->purchase_id }}" data-amount="{{ $payment->amount }}"
                    data-change="{{ $payment->change }}" data-paymentmethod="{{ $payment->payment_method }}"
                    data-accountid="{{ $payment->account_id }}" data-paymentno="{{ $payment->payment_no }}"
                    data-note="{{ $payment->payment_note }}"> <i class="fas fa-edit"></i></button>
                <button type="button" class="btn btn-danger btn-sm delete-payment" data-id="{{ $payment->id }}"
                    data-purchaseid="{{ $payment->purchase_id }}"> <i class="fas fa-trash"></i></button>
            </td>
        </tr>
    @endforeach
@else
